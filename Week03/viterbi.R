...
ViterbiFct <- function(InitProb,TransProb,EmisProb,ObsSeq){
  ...
  for (k in 2:Len){
    for (j in 1:nHS){
      # # # # cat("##################\n")
      # # # cat("TransProb j (1 to hiddenstates): ", TransProb[,j])
      # # cat("\nRow before: \n", MaxValue[k-1,])
      # cat("\nEMisson prob j, char at k: repeated nHS times:  ", rep(EmisProb[j,ObsSeq[k]],nHS))
      Val <- TransProb[,j]*MaxValue[k-1,]*rep(EmisProb[j,ObsSeq[k]],nHS)
      # cat("\n",Val)
